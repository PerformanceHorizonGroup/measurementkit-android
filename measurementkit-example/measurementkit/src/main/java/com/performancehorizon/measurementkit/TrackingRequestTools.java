...
public class TrackingRequestTools {
    ...
    {
        return TrackingRequestTools.nameValuePairsFromKeyAndValue(null, parametermap);
    }
    ...
    private static List<NameValuePair> nameValuePairsFromKeyAndValue(String key, Object value)
    {
        ...
        if (value instanceof Map<?, ?>)
        {
            ...
            for (Map.Entry<Object, Object> entry : map.entrySet())
            {
                ...
                if (nestedvalue != null) //sanity check, could happen.
                {
                    ...
                    {
                        returnparameterlist.addAll(TrackingRequestTools.nameValuePairsFromKeyAndValue(String.format("%s[%s]", key, nestedkey.toString()), nestedvalue));
                    }
                }
            }
        ...
        else if(value instanceof List<?>)
        {
            ...
            for (int i = 0; i < array.size(); i++)
            {
                ...
                returnparameterlist.addAll(TrackingRequestTools.nameValuePairsFromKeyAndValue(String.format("%s[%d]", key, i), nestedvalue));
            }
        }
